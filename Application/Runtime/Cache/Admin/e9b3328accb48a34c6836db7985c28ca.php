                ...
                <div class="panel-body">
                    <div class="row">
                        <div class="col-lg-6" style="width:100%;">
                            <form role="form" action="<?php echo U('Admin/Group/update');?>" method="post" enctype="multipart/form-data">
                                <div class="form-group">
                                    <label>权限组名称</label>
                                    <input name="title" type="text" value="<?php echo ($info['title']); ?>" class="form-control" >
                                </div>
                                 <div class="form-group">
                                    <label>选择所拥有的权限</label><br>
                                    <?php if(is_array($rules)): foreach($rules as $key=>$rule): ?><input type="checkbox"  name="rules[]" value="<?php echo ($rule['id']); ?>" <?php echo in_array($rule['id'],$info['rules']) ? 'checked' : '' ;?>>
                                        <span class="tdspan"><?php echo ($rule['title']); ?></span><?php endforeach; endif; ?>
                                </div>
