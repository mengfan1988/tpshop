...
<link rel="stylesheet" type="text/css" href="/20160727/buy/Public/Home/Css/myorder.css" media="all">
<script type="text/javascript" src="/20160727/buy/Public/Home/Js/my_order.js"></script>
    ...
    <li class="">
      ...
      <div class="g">
        ...
        <div class="gi one-whole my-oppo-content slab-white-border order">
          <?php if(is_array($xingxi)): foreach($xingxi as $key=>$vo): if($vo['fenlei'] == 0): ?><div class="order">
          <div class="g brick-shadow order-list-top">
            <div class="gi one-whole desk-one-half">
              订单编号：<strong><a class="clickable order-id" href="#"><?php echo ($vo['orderNum']); ?></a></strong>
            </div>
            <div class="gi one-whole desk-one-half desk-text-align-right">
              下单时间：<?php echo date('Y-m-d H-i-s',$vo['time']);?>
            </div>
          </div>
        ...
        <div class="order-list">
          <div class="order-item order-product desk-text-align-center">
            <a href="#" target="_top">
                    <img class="order-product-thumbnail" alt="" src="<?php echo ($vo['good']['pic']); ?>">
                  </a>
        <div class="row"><a href="#" target="_top"><?php echo ($vo['good']['phoneName']); ?></a></div>
                <p class="row">
              <a href="<?php echo U('Order/order_detail',array('id'=>$vo['id'],'orderNum'=>$vo['orderNum']));?>">+其余商品</a>
            </p>
              </div>
          <div class="order-item order-quantity desk-text-align-center">
           <span class="order-desk-hide">商品数量：</span><?php echo ($vo['count']); ?>
          </div>
          <div class="order-item order-amount desk-text-align-center">
            <span class="order-desk-hide">订单金额：</span>¥<?php echo ($vo['qian']); ?>
          </div>
          <div class="order-item order-detail">
            <div class="row order-username"><?php echo ($vo['dizhi']['shou']); ?></div>
            <div class="row"><?php echo ($vo['dizhi']['tel']); ?></div>
            <!-- <div class="row">1006131938@qq.com</div> -->
            <div class="row"><?php echo ($vo['dizhi']['address']); ?></div>
              </div>
          <div class="order-item order-primary desk-text-align-center">
            <div class="row">订单状态：
              <?php if($vo['huishou'] == 1): ?>已取消
                <?php elseif($vo['pay'] == 1): ?>已付款
                <?php elseif($vo['pay'] == 0): ?>未付款<?php endif; ?>


            </div>
                <?php if(($vo['pay'] == 0) and ($vo['huishou'] != 1)): ?><div class="row"><a class="button button-s fukuan" href="#" ordernum = "<?php echo ($vo['orderNum']); ?>" qian="<?php echo ($vo['qian']); ?>">立即支付</a></div><?php endif; ?>
                <div class="row"><a class="button button-s" href="<?php echo U('Person/order_detail',array('id'=>$vo['id'],'orderNum'=>$vo['orderNum']));?>">查看详情</a></div>
                <?php if(($vo['pay'] == 0) and ($vo['huishou'] == 0) ): ?><div class="row"><a class="box-link quxiao" href="#" orderid="<?php echo ($vo['id']); ?>">取消订单</a></div><?php endif; ?>
              </div>
        </div>
        </div><!-- jis -->
        ...
        <div class="row"><a href="#" target="_top"><?php echo ($vo['good']['partName']); ?></a></div>
                <p class="row">
              <a href="<?php echo U('Order/order_detail',array('id'=>$vo['id'],'orderNum'=>$vo['orderNum']));?>">+其余商品</a>
            </p>
              </div>
