...
$(function(){
	$('#button').click(function(){
		...
		$.post(	"/rule/index.php/Admin/Index/edit_pwd.html",{"old_pwd":$('#old_pwd').val(),"new_pwd":$('#new_pwd').val()},
			function(data){
				if(data == 1){
					layer.msg('修改成功，请重新登录...',{icon: 1,time: 2000,shade: [0.8, '#393D49']},function(){
							parent.window.location.href="<?php echo U('Public/login');?>";
						}
					);
				}else if(data == 2){
	  ...
	  <table width="100%" border="0" cellpadding="0" cellspacing="0" class="details">
        <tbody>
          ...
          <tr>
	      	...
	      	<td><input type="text" name="new_pwd" id="new_pwd" size="24" value="" />
	      	不能使用原密码！</td>
	      </tr>
          <tr>
            <td><div align="right">重复新密码：</div></td>
            <td><input type="text" name="new_pwd2" id="new_pwd2" size="24" value="" /></td>
          </tr>
        </tbody>
  </table>
</div>
